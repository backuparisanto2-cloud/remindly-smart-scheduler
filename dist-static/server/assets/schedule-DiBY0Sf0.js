...
function occurrencesFor(schedules, timeZone) {
	const out = [];
	for (const s of schedules) {
		const time = (s.send_time ?? "09:00").slice(0, 5);
		if (!s.start_date) continue;
		if (s.kind === "range" && s.end_date) for (const day of eachDate(s.start_date, s.end_date)) {
			const dow = (/* @__PURE__ */ new Date(`${day}T00:00:00Z`)).getUTCDay();
			if (!s.weekdays?.length || s.weekdays.includes(dow)) out.push(zonedToUtc(day, time, timeZone));
		}
		else out.push(zonedToUtc(s.start_date, time, timeZone));
	}
	return out.sort((a, b) => a.getTime() - b.getTime());
}
//#endregion
export { zonedToUtc as n, occurrencesFor as t };
